...
function getSetting(key1) {
    if (settings[key1]) return settings[key1];
    else {
        settings[key1] = defaultSettings[key1];
        saveSettings();
        return defaultSettings[key1];
    }
}
function setSetting(key1, value) {
    settings[key1] = value;
    saveSettings();
}
...
function connect() {
    ...
    socket.io.on("match_state", (data)=>{
        ...
        for (let move of game.gameData.moves)game.localData.board = makeMove(move.move, game.localData.board, Math.seedrandom, game.gameData.match.rules);
        game.localData.board.move = moveNumber;
        if (game.gameData.match.newNotification) game.boardNotifications.push({
...
function updateInput() {
    for(let key1 in input.keys)if (input.keys[key1] >= 0) input.keys[key1] += 1;
...
function update() {
    ...
    if (socket.registered) {
        game.mainMenuAnim = lerp(game.mainMenuAnim, 0, clamp(6 * deltaTime, 0, 1));
        game.menuAnim = lerp(game.menuAnim, 0, clamp(6 * deltaTime, 0, 1));
        game.matchAnim = lerp(game.matchAnim, 0, clamp(6 * deltaTime, 0, 1));
        if (!game.inGame) {
            if (state.joiningMatch) {
                ...
                if (input.mouse[LEFT] === 0 && mouseOver) game.menu[i].onClick();
            }
        }
    }
    if (game.inGame) {
        ...
        if (!game.gameData.match.playing || game.gameData.match.players.filter((id)=>!game.gameData.match.boards[id].lost).length <= 1) origin = {
            x: width / 2,
            y: height / 2
        };
        else origin = {
        ...
        game.boardOrigin[0] = lerp(game.boardOrigin[0], origin.x, clamp(6 * deltaTime, 0, 1));
        game.boardOrigin[1] = lerp(game.boardOrigin[1], origin.y, clamp(6 * deltaTime, 0, 1));
        game.boardTranslation[0] = lerp(game.boardTranslation[0], 0, clamp(6 * deltaTime, 0, 1));
        game.boardTranslation[1] = lerp(game.boardTranslation[1], 0, clamp(6 * deltaTime, 0, 1));
        game.boardRotation = lerp(game.boardRotation, 0, clamp(6 * deltaTime, 0, 1));
        ...
        if (game.gameData.match.playing) {
            ...
            for (let id of game.gameData.match.players.filter((id)=>id !== game.playerData._id)){
                ...
                game.localData.boardAnims[id][0] = lerp(game.localData.boardAnims[id][0], idealPos[0], clamp(6 * deltaTime, 0, 1));
                ...
                if (isSpectator) spectating = false;
            }
        }
    }
}
    ...
    if (detail >= 4) {
        ...
        if (count > 0) {
            ...
            for (let combo of combos){
                ...
                if (sumLines <= 1) fill(30, 50, 150);
                else if (sumLines <= 2) fill(140, 60, 10);
                else if (sumLines <= 3) fill(80, 30, 140);
                else if (sumLines <= 4) fill(30, 150, 130);
                else if (sumLines <= 5) fill(140, 130, 10);
        ...
        noStroke();
        fill(0, 100);
        if (board.finishingMoves >= 0) text(board.finishingMoves, 0, 0);
    }
}
...
function drawState() {
    ...
    if (!game.gameData.match.playing) {
        ...
        for(let i4 = 0; i4 < game.gameData.match.players.length; i4++){
            ...
            if (game.gameData.match.ready[id]) fill(100, 250, 120);
            else fill(250, 120, 100);
            rect(width - 220, 10 + y, 10, 40);
        }
        if (game.localData.ready) fill(100, 250, 120);
        else fill(250, 120, 100);
        textSize(12);
        textAlign(RIGHT, CENTER);
        textStyle(BOLD);
...
//# sourceMappingURL=index.a8b85b30.js.map
